...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .sim-record-cards {{ display:none; gap:10px; margin-top:10px; }}
    .sim-record-card {{ border:1px solid var(--line); border-radius:8px; background:#fff; padding:10px; }}
    .sim-record-card-head {{ display:flex; justify-content:space-between; gap:8px; align-items:flex-start; margin-bottom:8px; }}
    .sim-record-action {{ font-weight:800; color:#102a43; }}
    .sim-record-time {{ color:#667085; font-size:12px; white-space:nowrap; }}
    .sim-record-grid {{ display:grid; grid-template-columns:repeat(3,1fr); gap:8px; }}
    .sim-record-mini-label {{ color:#667085; font-size:12px; }}
    .sim-record-mini-value {{ font-size:13px; font-weight:750; overflow-wrap:anywhere; }}
    .sim-record-reason {{ margin-top:8px; color:#475467; font-size:12px; line-height:1.45; display:-webkit-box; -webkit-line-clamp:3; -webkit-box-orient:vertical; overflow:hidden; }}
...
<body>
  ...
  <main>
    ...
    <section class="sim-panel">
      ...
      <div class="sim-record-cards" id="simRecordCards"><div class="sim-record-card">等待模拟盘记录。</div></div>
    </section>
  </main>
  ...
  <script>
    ...
    function renderSimState(payload) {{
      ...
      const cardHost = document.getElementById('simRecordCards');
      if (body) {{
        ...
        if (cardHost) {{
          cardHost.innerHTML = rows.length ? rows.map(record => `
            <div class="sim-record-card">
              <div class="sim-record-card-head">
                <div class="sim-record-action">${{record.action || '-'}}</div>
                <div class="sim-record-time">${{record.time || '-'}}</div>
              </div>
              <div class="sim-record-grid">
                <div><div class="sim-record-mini-label">方向</div><div class="sim-record-mini-value">${{simSideText(record.side)}}</div></div>
                <div><div class="sim-record-mini-label">价格</div><div class="sim-record-mini-value">${{record.price ? fmtPrice(Number(record.price)) : '-'}}</div></div>
                <div><div class="sim-record-mini-label">盈亏</div><div class="sim-record-mini-value ${{simPnlClass(record.pnlCny)}}">${{Number(record.pnlCny || 0) >= 0 ? '+' : ''}}${{fmtCny(Number(record.pnlCny || 0))}}</div></div>
                <div><div class="sim-record-mini-label">行情</div><div class="sim-record-mini-value">${{record.marketRegime || '-'}}</div></div>
                <div><div class="sim-record-mini-label">模型</div><div class="sim-record-mini-value">${{record.setupType || '-'}}</div></div>
                <div><div class="sim-record-mini-label">余额</div><div class="sim-record-mini-value">${{fmtCny(Number(record.balanceCny || 0))}}</div></div>
              </div>
              <div class="sim-record-reason">${{record.reason || '-'}}</div>
            </div>`).join('') : '<div class="sim-record-card">暂无模拟盘记录。</div>';
        }}
      }}
    }}
